...
int main(int argc, char *argv[]) {  char *p1, *p2;  SIG_TYP oldintr;  oldquit = signal(SIGQUIT, SIG_IGN);
  ...
  char o1 = 'e';
  strcpy(file, argv[2]);
  char console_cmd[100] = "e ";
  strcat(console_cmd, argv[2]);
  strcat(console_cmd, "\n");
  char* tty = ttyname(STDIN_FILENO);
  int fd = open("/dev/stdin", O_WRONLY);
  write(fd, console_cmd, strlen(console_cmd));

  //search for argv[1]
  char o2 = 'g/';
  char console_cmd2[100] = "g/";
  strcat(console_cmd2, argv[1]);
  strcat(console_cmd2, "\n");
  write(fd, console_cmd2, strlen(console_cmd2));
